...
import { Header, Footer, FeaturedAuctions, SidebarFilters } from "@/components";
...
export default function SearchPage() {
    const [filters, setFilters] = useState<SidebarFilterState>(DEFAULT_SIDEBAR_FILTERS);
    ...
    const searchResults = useMemo(() => {
        return ALL_AUCTIONS.filter((auction) => {
            ...
            // 3. Variant
            if (filters.variant) {
                const variantParts = filters.variant.toLowerCase().split("-");
    ...
    const activeFilters = useMemo(() => {
        ...
        const isDefault = <K extends keyof SidebarFilterState>(key: K, val: any) => {
            return JSON.stringify(val) === JSON.stringify(DEFAULT_SIDEBAR_FILTERS[key]);
        };
        ...
        // Year
        if (filters.yearMin !== DEFAULT_SIDEBAR_FILTERS.yearMin || filters.yearMax !== DEFAULT_SIDEBAR_FILTERS.yearMax) {
        ...
        // Price
        if (filters.priceMin !== DEFAULT_SIDEBAR_FILTERS.priceMin || filters.priceMax !== DEFAULT_SIDEBAR_FILTERS.priceMax) {
    ...
    const handleRemoveFilter = (filterId: string) => {
        if (filterId === "year") {
            setFilters(prev => ({
                ...prev,
                yearMin: DEFAULT_SIDEBAR_FILTERS.yearMin,
                yearMax: DEFAULT_SIDEBAR_FILTERS.yearMax
            }));
            return;
        }
        if (filterId === "price") {
            setFilters(prev => ({
                ...prev,
                priceMin: DEFAULT_SIDEBAR_FILTERS.priceMin,
                priceMax: DEFAULT_SIDEBAR_FILTERS.priceMax
            }));
            return;
        }
        if (filterId === "mileage") {
        ...
        if (filter) {
            handleFilterChange(filter.key, DEFAULT_SIDEBAR_FILTERS[filter.key]);
        }
    };
                ...
                >
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 relative items-start mt-6">
                        ...
                        <div className="flex-1">
                            {/* Pagination Placeholder if needed at top or just content */}
